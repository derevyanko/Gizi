...
class StartWindow():
	def __init__(self, size):
		self.size = size
		self.texts = Texts([Text('Играть', (215, 200)), Text('Результаты', (190, 250)), 
			Text('Выход', (215, 300))])
		self.screen = pygame.display.set_mode(size)

	def draw(self, window, timer):
		self.get_events(window)
		window.windows[window.status].screen.fill((0, 0, 0))

		font = self.texts.texts[0].font
...
class GamePlayWindow():
	...
	def gamePause(self, window):
		self.get_eventsPauseGame(window)
		window.windows['gameplay'].drawPause(window)
	...
	def drawPause(self, window):
		...
		font = self.texts['pauseGame'].texts[0].font
		...
		if self.texts['pauseGame'].pointer == 0:
			first_text = font.render(self.texts['pauseGame'].texts[0].text, 5, (0, 0, 0))
			second_text = font.render(self.texts['pauseGame'].texts[1].text, 5, (255, 255, 255))
			third_text = font.render(self.texts['pauseGame'].texts[2].text, 5, (255, 255, 255))
			rect_text = first_text.get_rect()
			...
			first_text = font.render(self.texts['pauseGame'].texts[0].text, 5, (255, 255, 255))
			second_text = font.render(self.texts['pauseGame'].texts[1].text, 5, (0, 0, 0))
			third_text = font.render(self.texts['pauseGame'].texts[2].text, 5, (255, 255, 255))
			rect_text = second_text.get_rect()
			...
			first_text = font.render(self.texts['pauseGame'].texts[0].text, 5, (255, 255, 255))
			second_text = font.render(self.texts['pauseGame'].texts[1].text, 5, (255, 255, 255))
			third_text = font.render(self.texts['pauseGame'].texts[2].text, 5, (0, 0, 0))
			rect_text = third_text.get_rect()
	...
	def get_eventsPauseGame(self, window):
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				window.run = False
			elif event.type == pygame.KEYDOWN:
...
class InputBox:
    ...
    def render(self, window):
        ...
        draw.rect(window.windows[window.status].screen, colors['black'], (self.x, self.y, self.width, self.height))
